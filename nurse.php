...
<div id="menu" class="absolute bg-white w-64 p-4 hidden top-[65px] left-0 shadow z-10 rounded-md">
    ...
    <a class="block mb-4 block hover:bg-gray-200 rounded px-1" href="medication_dashboard.php">📁 Medical Supplies</a>
    <a class="block mb-4 block hover:bg-gray-200 rounded px-1" href="studentfile_dashboard.php">📁 Student File Dashboard</a>
    <a class="block mb-4 block hover:bg-gray-200 rounded px-1" href="registration.php">📁 Register Student Health</a>
    <a class="block mb-4 block hover:bg-gray-200 rounded px-1" href="Student_visitlogs.php">📁 Student Visit Logs</a>
    <a class="block mb-4 block hover:bg-gray-200 rounded px-1" href="appointment_history.php">📁 Appointment History</a>
    <a class="block mb-4 block hover:bg-gray-200 rounded px-1" href="emergency_reports.php">📁 Emergency Reports</a>
    <a class="block mb-4 block hover:bg-gray-200 rounded px-1" href="responder_status.php">📁 Responder Status</a>
    <a class="block mb-4 block hover:bg-gray-200 rounded px-1" href="nurse_reset_password.php">🔑 Reset User Password</a>
...
<main class="max-w-6xl mx-auto mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
  ...
  <div class="col-span-2 space-y-4">
    <div class="bg-pink-100 p-4 rounded-md shadow">
      ...
      <div id="appointments" class="mt-2 max-h-[400px] overflow-y-auto transition-all duration-300">
              ...
              <?php while($row = $result_appt->fetch_assoc()): ?>
                <tr id="row-<?php echo $row['id']; ?>">
                  ...
                  <td class="p-2 flex items-center space-x-1">
                    <button class="view-details-btn bg-green-500 text-white px-2 py-1 rounded text-sm" data-id="<?php echo $row['id']; ?>" data-student="<?php echo htmlspecialchars($row['student_name']); ?>" data-reason="<?php echo htmlspecialchars($row['reason']); ?>" data-date="<?php echo $row['appointment_time']; ?>">Details</button>
...
<div id="followupModal" class="fixed inset-0 bg-black bg-opacity-50 hidden flex items-center justify-center">
  <div class="bg-white p-6 rounded shadow-lg w-96">
    ...
    <div class="flex justify-end gap-2">
      <button type="button" id="skipBtn" onclick="skipFollowup()" class="px-3 py-1 bg-gray-400 text-white rounded">Skip</button>
      <button type="button" onclick="submitFollowup()" class="px-3 py-1 bg-green-500 text-white rounded">Schedule</button>
    </div>
  </div>
</div>
...
function displayMedicineEntries(entries) {
  ...
  if (entries.length > 0) {
    entries.forEach(entry => {
      ...
      entryDiv.className = 'bg-gray-50 p-3 rounded shadow mb-4';
      entryDiv.innerHTML = `
        <p class="text-sm text-gray-600 mb-2"><strong>Date:</strong> ${new Date(entry.created_at).toLocaleString()}</p>
        <p><strong>Medicine Name:</strong> ${entry.medicine_name}</p>
        <p><strong>Dosage:</strong> ${entry.dosage || 'N/A'}</p>
        <p><strong>Quantity:</strong> ${entry.quantity}</p>
        <p><strong>Action Taken:</strong> ${entry.action_taken || 'N/A'}</p>
        <div class="flex justify-end gap-2 mt-2">
          <button type="button" onclick="deleteMedicineEntry(${entry.id})" class="px-3 py-1 bg-red-500 text-white rounded">Delete</button>
        </div>
      `;
  ...
  // Always display the editable form for adding/updating
  let entry = null;
  ...
  if (entries.length > 0) {
    // Populate with the most recent entry for editing
  ...
  formDiv.innerHTML = `
    ...
    <div class="flex justify-end gap-2">
      ...
      <button type="button" onclick="closeDetailsModal()" class="px-3 py-1 bg-gray-500 text-white rounded">Cancel</button>
    </div>
  `;
...
async function deleteMedicineEntry(entryId) {
  ...
  try {
    ...
    if (data.success) {
      ...
      alert("Failed to delete medicine entry: " + (data.message || "Unknown error"));
    }
  } catch (err) {
    console.error("Error deleting medicine entry:", err);
    ...
    if (data.length > 0) {
      data.forEach(row => {
        ...
        tr.innerHTML = `
          ...
          <td class="p-2 flex items-center space-x-1">
            <button class="view-details-btn bg-green-500 text-white px-2 py-1 rounded text-sm" data-id="${row.id}" data-student="${row.student_name}" data-reason="${row.reason}" data-date="${row.appointment_time}">Details</button>
...
async function handleAction(id, action) {
  try {
    if (action === 'done') {
      // Check if medicine entry exists for this appointment
      try {
        const checkRes = await fetch(`fetch_medicine_entries.php?appointment_id=${id}`);
        const checkData = await checkRes.json();
        if (!checkData.success || checkData.entries.length === 0) {
          alert("Please enter medicine details in the appointment details before marking as done.");
          return;
        }
      } catch (err) {
        console.error("Error checking medicine entries:", err);
        alert("Unable to verify medicine details. Please try again.");
        return;
      }
...
function toggleExpand(id, btn) {
    ...
    // If expanded already → collapse
    if (box.classList.contains("max-h-full")) {
        box.classList.remove("max-h-full");
        box.classList.add("max-h-[400px]", "overflow-y-auto");
        btn.textContent = "View all >";
    } else {
        // Expand
        box.classList.remove("max-h-[400px]", "overflow-y-auto");
        box.classList.add("max-h-full");
